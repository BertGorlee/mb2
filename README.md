...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///mb2` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE mb2 WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE mb2 TO ;
```
...
ERROR:  database "mb2" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists mb2
